      ...
      dimension: creative_name {
        ...
        sql:
              case
                ...
                when ${promoted_pin_name} ilike '%_TorreyPines' then 'Torrey Pines'
        when ${promoted_pin_name} ilike '%_LeopardSharks' then 'Swimming with Leopard Sharks'
        when ${promoted_pin_name} ilike '%TidePools' then 'SD Tide Pools: Exploring Life at the Edge of the Sea'
        when ${promoted_pin_name} ilike '%_Sailing' then 'Sailing in San Diego'
        when ${promoted_pin_name} ilike '%_MissionBayBeaches' then 'Mission Bay & Beaches'
        when ${promoted_pin_name} ilike '%_MeleSurfFaves' then 'Mele Sali''s Surf Faves'
        when ${promoted_pin_name} ilike '%_LaJollaBeaches' then 'La Jolla Beaches'
        when ${promoted_pin_name} ilike '%_HistoricHighway101' then 'Historic Highway 101'
        when ${promoted_pin_name} ilike '%_GuidetoSDBeaches' then 'Guide to San Diego Beaches'
          when ${promoted_pin_name} ilike '%SocalVibe%' then '3 Beach Neighborhoods w/ SoCal Vibe'
          ...
          when ${promoted_pin_name} ilike '%PacificBeach%' then 'Pacific Beach'
          when ${promoted_pin_name} ilike '%BirdRock%' then 'Bird Rock'
          ...
          when ${promoted_pin_name} ilike '%25ThingsToDo%' then '25 Fun & Free Things to do'
          when ${promoted_pin_name} ilike '%15SanDiegoPlaygrounds%' then '15 San Diego Playgrounds'
          when ${promoted_pin_name} ilike '%13Kidneighborhoods%' then '13 Kid-Pleasing Neighborhoods'
          when ${promoted_pin_name} ilike '%BestBeaches%' then 'The Best Beaches According to Kids'
          when ${promoted_pin_name} ilike '%Familyfun%' then 'Family Fun in San Diego'
          when ${promoted_pin_name} ilike '%Edutainment%' then 'Edutainment'
          when ${promoted_pin_name} ilike '%KidApprovedParks%' then '9 Kid Approved Parks'
          when ${promoted_pin_name} ilike '%FoodieFinds%' then 'Foodie Finds'
          when ${promoted_pin_name} ilike '%Azcuar%' then 'Azucar'
          ...
          when ${promoted_pin_name} ilike '%CrossBorderCuisine%' then 'Cross-Border Cuisine'
          when ${promoted_pin_name} ilike '%SeafoodRestaurants%' then 'Best Seafood Restaurants'
          when ${promoted_pin_name} ilike '%Realmof52Remedies%' then 'Realm of the 52 Remedies'
          when ${promoted_pin_name} ilike '%59MileScenicDrive%' then 'San Diego''s 59 Mile Scenic Drive'
          ...
          when ${promoted_pin_name} ilike '%BalboaParkCulturalHeart%' then 'Balboa Park - Cultural Heart'
          ...
          when ${promoted_pin_name} ilike '%LaJollaKayak%' then 'Explore La Jolla on a Kayak Tour'
