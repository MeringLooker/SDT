  ...
  dimension: creative_name {
    ...
    sql:
      case
        ...
        when ${ad_group} ilike '%_TorreyPines%' then 'Torrey Pines'
        when ${ad_group} ilike '%_LeopardSharks%' then 'Swimming with Leopard Sharks'
        when ${ad_group} ilike '%TidePools%' then 'SD Tide Pools: Exploring Life at the Edge of the Sea'
        when ${ad_group} ilike '%_Sailing%' then 'Sailing in San Diego'
        when ${ad_group} ilike '%_MissionBayBeaches%' then 'Mission Bay & Beaches'
        ...
        when ${ad_group} ilike '%_GuidetoSDBeaches%' then 'Guide to San Diego Beaches'
          ...
          when ${ad_group} ilike '%PacificBeach%' then 'Pacific Beach'
          when ${ad_group} ilike '%BirdRock%' then 'Bird Rock'
          ...
          when ${ad_group} ilike '%25FreeFunThings%' then '25 Fun & Free Things to do'
          when ${ad_group} ilike '%15SanDiegoPlaygrounds%' then '15 San Diego Playgrounds'
          when ${ad_group} ilike '%13Kidneighborhoods%' then '13 Kid-Pleasing Neighborhoods'
          when ${ad_group} ilike '%BestBeaches%' then 'The Best Beaches According to Kids'
          when ${ad_group} ilike '%Familyfun%' then 'Family Fun in San Diego'
          when ${ad_group} ilike '%Edutainment%' then 'Edutainment'
          when ${ad_group} ilike '%KidApprovedParks%' then '9 Kid Approved Parks'
          when ${ad_group} ilike '%FoodieFinds%' then 'Foodie Finds'
          when ${ad_group} ilike '%Azcuar%' then 'Azucar'
          ...
          when ${ad_group} ilike '%CrossBorderCuisine%' then 'Cross-Border Cuisine'
          when ${ad_group} ilike '%SeafoodRestaurants%' then 'Best Seafood Restaurants'
          when ${ad_group} ilike '%Realmof52Remedies%' then 'Realm of the 52 Remedies'
          when ${ad_group} ilike '%59MileScenicDrive%' then 'San Diego''s 59 Mile Scenic Drive'
          ...
          when ${ad_group} ilike '%BalboaParkCulturalHeart%' then 'Balboa Park - Cultural Heart'
          when ${ad_group} ilike '%Sunny7makeaSplash%' then 'S7: Make A Splash'
          when ${ad_group} ilike '%Sunny7Sunsets%' then 'S7: Sunsets'
          when ${ad_group} ilike '%LaJollaKayak%' then 'Explore La Jolla on a Kayak Tour'
